...
**分步保存带乐观锁。** `PATCH` 支持 `If-Match: <version>` 头。版本不符返回 409 并在 `meta.currentVersion` 里给出当前版本。响应的 `ETag` 头回带新版本号，客户端下一次直接拿它做 `If-Match`。

用 `If-Match` 而不是把 version 塞进请求体，是因为这是 HTTP 早就定义好的条件请求语义，代理和网关都认识它，不必自造约定。

省略 `If-Match` 时退化为后写覆盖，但仍会递增版本号并写入流水，数据不会损坏。这是给简单客户端留的口子，代价明确写在这里。
...
**Playwright 端到端。** 已规划但尚未接入。集成测试已经覆盖了 route handler 层的完整链路，端到端的增量价值主要在「浏览器里点得动」，属于优先级更低的一层。
